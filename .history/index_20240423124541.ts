...
app.post('/enchiper/mint', (req: Request, res: Response) => {
  const enchiper: EnchiperData = req.body;
  
  if (enchiper.method === 'mint'){
    const runestone = new Runestone({
      edicts: [
        {
          id: new RuneId(new U64(enchiper.block), new U32(enchiper.tx_index)),
          output: new U32(enchiper.outInex),
        },
      ],
      mint: new RuneId(new U64(enchiper.block), new U32(enchiper.tx_index)),
    })
    
  }else if (enchiper.method === 'transfer'){
    const runestone = new Runestone({
      edicts: [
        {
          id: new RuneId(new U64(enchiper.block), new U32(enchiper.tx_index)),
          amount: new U128(enchiper.amount),
          output: new U32(enchiper.outInex),
        },
      ],
      mint: new RuneId(new U64(enchiper.block), new U32(enchiper.tx_index)),
