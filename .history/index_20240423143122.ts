...
app.post('/enchiper/mint', (req: Request, res: Response) => {
  ...
  const runes = new Runestone({
    edicts: [
      {
        id: new RuneId(U64.fromString(enchipermint.block), U32.fromString(enchipermint.tx_index)),
        output: U32.fromString(enchipermint.out_index),
...
app.post('/enchiper/transfer', (req: Request, res: Response) => {
  const enchiperData: EnchiperTransfer = req.body;
  const runes = new Runestone({
    edicts: [
      {
